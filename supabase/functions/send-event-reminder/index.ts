...
Deno.serve(async (req) => {
  ...
  try {
    // SECURITY: Verify this is a service role request (server-side invocation only)
    // This endpoint is designed to be called by pg_cron or admin tools, not by browsers
    const authHeader = req.headers.get("Authorization");
    const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
    
    // Use strict equality check - must be exactly "Bearer {service_key}"
